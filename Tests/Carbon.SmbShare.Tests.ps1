...
BeforeAll {
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'Carbon.SmbShare'|
...
Describe 'Carbon.SmbShare' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.SmbShare' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.SmbShare' -CommandType Function,Cmdlet,Filter))
        {
